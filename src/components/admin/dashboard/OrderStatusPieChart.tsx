...
type OrderStatusResponse = {
  success: boolean;
  data: { counts: OrderStatusCounts; total: number };
};
...
const SEGMENTS: {
  ...
  { key: "requested", name: "Requested", color: "var(--warning)" },
  { key: "accepted", name: "Accepted", color: "var(--primary)" },
  { key: "rejected", name: "Rejected", color: "var(--destructive)" },
];
...
export function OrderStatusPieChart() {
  ...
  useEffect(() => {
    let cancelled = false;
    api<OrderStatusResponse>("/admin/dashboard/order-status")
      .then((res) => {
        if (!cancelled) {
